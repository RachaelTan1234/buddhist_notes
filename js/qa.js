...
async function fetchQA() {
  ...
  // 精确判断：当前用户必须明确是“师父”角色（防止普通同修看到回复框）
  const isMaster = currentUser && currentUser.role === '师父';

  container.innerHTML = (data || []).map(item => {
    const canDelete = currentUser && (currentUser.role === '管理员' || currentUser.role === '师父' || item.username === currentUsername);
    ...
    return `
      <div class="card" style="position:relative; background:#FFF; border:1px solid #E5E7EB; border-radius:8px; padding:12px; margin-bottom:12px;">
        ...
        <!-- 只有登录角色为“师父”且该提问还没有开示时，才显示专属回复输入框；普通同修绝对看不到 -->
